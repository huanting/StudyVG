...
public class FileUtil {
    ...
    public static boolean deleteDirectory(File path) {
        ...
        Log.i("test", "deleteDirectory file=" + path.getPath() + ", exist=" + path.exists());
        ...
        for (File f : files) {
            if (f.isDirectory()) {
                if (!deleteDirectory(f)) {
                    Log.e("test", "Fail to delete folder: " + f.getPath());
                    return false;
                }
            } else {
                if (!f.delete()) {
                    Log.e("test", "Fail to delete file: " + f.getPath());
                    return false;
                }
                Log.i("test", "deleteFiles =" + f.getPath());
            }
        }
        return true;
    }
