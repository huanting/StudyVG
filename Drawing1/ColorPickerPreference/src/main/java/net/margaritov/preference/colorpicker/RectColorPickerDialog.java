...
public class RectColorPickerDialog
        extends
        Dialog
        implements
        RectColorPickerView.OnColorChangedListener,
        View.OnClickListener, ViewTreeObserver.OnGlobalLayoutListener {

    private RectColorPickerView mColorPicker;

    private RectColorPickerPanelView mOldColor;
    private RectColorPickerPanelView mNewColor;
    ...
    public RectColorPickerDialog(Context context, int initialColor) {
        super(context);
    ...
    private void setUp(int color) {
        ...
        mColorPicker = (RectColorPickerView) mLayout.findViewById(R.id.color_picker_view);
        mOldColor = (RectColorPickerPanelView) mLayout.findViewById(R.id.old_color_panel);
        mNewColor = (RectColorPickerPanelView) mLayout.findViewById(R.id.new_color_panel);
            ...
            public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
                if (actionId == EditorInfo.IME_ACTION_DONE) {
                    ...
                    if (s.length() > 5 || s.length() < 10) {
                        try {
                            int c = RectColorPickerPreference.convertToColorInt(s.toString());
    ...
    private void updateHexValue(int color) {
        if (getAlphaSliderVisible()) {
            mHexVal.setText(RectColorPickerPreference.convertToARGB(color).toUpperCase(Locale.getDefault()));
        } else {
            mHexVal.setText(RectColorPickerPreference.convertToRGB(color).toUpperCase(Locale.getDefault()));
        }
        mHexVal.setTextColor(mHexDefaultTextColor);
    }
