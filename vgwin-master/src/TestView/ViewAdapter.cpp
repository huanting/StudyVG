...
//开始记录
bool ViewAdapter::startRecord(const char* path)
{
	return _coreView->startRecord(path, _coreView->acquireFrontDoc(),false, GetTickCount());
}

//停止记录 
void ViewAdapter::stopRecord(const char* path)
{
	_coreView->stopRecord(false);
	_coreView->saveToFile(_coreView->acquireFrontDoc(), path, true);
}

 //播放

void ViewAdapter::play(const char* path)
{
	_coreView->loadFromFile(path);
}